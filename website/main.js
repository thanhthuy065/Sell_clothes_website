...
// }
function toImgUrl(path) {
  if (!path) return IMG_BASE + '/uploads/placeholder.png';
  ...
  // Ảnh cũ dạng /image/ao/..., /image/vayngan/...
    ...
    <div class="product-card" onclick="viewProductDetail('${encodeURIComponent(JSON.stringify(p))}')">
      <div class="product-thumb">
        <img loading="lazy" src="${toImgUrl(p.image_url)}" alt="${p.product_name}"
             onerror="this.onerror=null;this.src='${IMG_BASE}/uploads/placeholder.png'">
      </div>
...
async function loadProducts() {
  try {
    ...
    //   const title = ({
    //     'vay-ngan':'VÁY NGẮN','vay-dai':'VÁY DÀI','ao':'ÁO','chan-vay':'CHÂN VÁY','set-bo':'SET BỘ'
    //   }[cat]) || (cat||'').replace(/-/g,' ').toUpperCase();

    //   setText('pageTitle', title); setText('categoryTitle', title);
    //   updateProductGrid('categoryGrid', list);
    //   return;
    // }
...
//   // Map slug -> từ khoá tiếng Việt + thư mục ảnh
//   const MAP = {
//     //'best-seller': { kw: 'best seller', dirs: [] }, // special case
//     'vay-ngan': { kw: 'váy ngắn', dirs: ['VayNgan'] },
//     'vay-dai' : { kw: 'váy dài',  dirs: ['VayDai']  },
//     'ao'      : { kw: 'áo',       dirs: ['Ao','AoKieu'] },
//     'chan-vay': { kw: 'chân váy', dirs: ['ChanVay'] },
//     'set-bo'  : { kw: 'set',      dirs: ['SetBo'] }
//   };
//   const cfg = MAP[cat] || { kw: cat, dirs: [] };
...
//   // 1) Thử gọi API lọc theo từ khoá tiếng Việt
//   const url = new URL(`${API_BASE}/products`);
//   if (cfg.kw) url.searchParams.set('category', cfg.kw);
//   if (sort && sort !== 'all') url.searchParams.set('sort', sort);
...
//   // 2) Fallback: nếu rỗng, lấy all rồi lọc client-side theo dirs + tên
//   if (!Array.isArray(list) || list.length === 0) {
//     try {
//       const rAll = await fetch(`${API_BASE}/products`);
//       const all  = await rAll.json();

//       const inDirs = (p, dirs) => {
//         const u = (p.image_url||'').toLowerCase().replace(/\\/g,'/');
//         return dirs.length && dirs.some(d => u.includes('/' + d.toLowerCase() + '/'));
//       };
//       const byName = (p, kw) => unsign(p.product_name).includes(unsign(kw));

//       list = all.filter(p => inDirs(p, cfg.dirs) || byName(p, cfg.kw));
//     } catch (e) {
//       console.error('fallback filter error', e);
//       list = []; // giữ rỗng nếu có sự cố
//     }
//   }
...
//   // Trường hợp đặc biệt: trang "Best seller" như một danh mục
//   if (cat === 'best-seller') {
//     const r = await fetch(`${API_BASE}/products`);
//     const all = await r.json();
//     const best = pickBestSellers(all, 12); // muốn nhiều hơn 8 thì tăng số này

//     setText('pageTitle', 'BEST SELLER');
//     setText('categoryTitle', 'BEST SELLER');
//     updateProductGrid('categoryGrid', best);
//     return;
//   }

//   // Các danh mục còn lại: map slug -> từ khoá tiếng Việt
//   const kwMap = {
//     'vay-ngan': 'váy ngắn',
//     'vay-dai' : 'váy dài',
//     'ao'      : 'áo',
...
//   const url = new URL(`${API_BASE}/products`);
//   if (keyword) url.searchParams.set('category', keyword);
//   if (sort && sort !== 'all') url.searchParams.set('sort', sort);
...
//   // Fallback nếu BE chưa hỗ trợ: lấy all rồi lọc client-side
//   if (!Array.isArray(list) || list.length === 0) {
//     const rAll = await fetch(`${API_BASE}/products`);
//     const all  = await rAll.json();
//     const unsign = s => String(s||'').normalize('NFD').replace(/[\u0300-\u036f]/g,'').toLowerCase();
//     list = all.filter(p => unsign(p.product_name).includes(unsign(keyword)));
//   }
...
// ----- COLLECTION PAGE -----
if (/collections\.html$/i.test(location.pathname)) {
    ...
    // 2. Lấy sản phẩm đúng theo bộ sưu tập
    let url = `${API_BASE}/products?collection=${id}`;
