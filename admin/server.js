...
app.post('/api/products', (req, res) => {
  upload.single('image')(req, res, async (err) => {
    ...
    // ---- Xử lý ảnh: ưu tiên file, fallback imageUrl ----
    let imagePath = null;
...
// ================== API THÊM SẢN PHẨM BẰNG JSON (POSTMAN) ==================
  ...
  try {
    ...
    await pool.request()
      ...
      .query(`
        ...
        (product_name, price, image_url, price_promotion, detail, collection_id, status)
        VALUES
        (@product_name, @price, @image_url, @price_promotion, @detail, @collection_id, @status)
      `);
...
app.put('/api/products/:id', upload.single('image'), async (req, res) => {
  try {
    ...
    const checkCollection = await req.pool.request()
      .input('collection_id', sql.Int, collection_id)
      .query('SELECT 1 FROM Collections WHERE collection_id = @collection_id');
